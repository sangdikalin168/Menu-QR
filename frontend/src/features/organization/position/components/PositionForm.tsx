...
type PositionItem = { id?: string; name?: string; description?: string };

export const PositionForm: React.FC<{ item?: PositionItem; onSubmit: (v: { name: string; description?: string }) => Promise<{ success: boolean }>; onClose: () => void }> = ({ item, onSubmit, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 flex items-center justify-center bg-white/10 backdrop-blur-sm z-50">
            <form onSubmit={handleSubmit} className="bg-white p-6 rounded shadow w-96">
                <h2 className="text-lg font-semibold mb-4">{item ? 'Edit' : 'Add'} Position</h2>
